...
void ofApp::setup(){
    ...
    //set up aggregator
    if(bScaleDown){
        stitcher.setup(numFeeds, feedWidth/2, feedHeight/2);
    } else {
        stitcher.setup(numFeeds, feedWidth, feedHeight);
    }

    stitcher.adjustedOrigin = rawImagePos;
...
void ofApp::update(){
    ...
    if(viewMode >= 0 && viewMode < numFeeds){
        ...
        stitcher.bEnableMouse = false;
        
    } else if(viewMode == 6){
        
        stitcher.bEnableMouse = true;
    ...
    //now pass the raw camera textures and the thresholded pixels to the aggregator
    stitcher.getNewData(camTexs, camPixels);
...
void ofApp::draw(){
    ...
    if(viewMode >= 0 && viewMode < numFeeds){
        ...
        stitcher.drawRaw(rawImagePos.x, rawImagePos.y);
        stitcher.drawCV(rawImagePos.x, rawImagePos.y);
        
        stitcher.aGui.draw(15, topMargin);
