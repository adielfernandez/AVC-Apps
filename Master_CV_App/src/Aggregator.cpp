...
//        maskPix.pasteInto(newMask, 0, 0);
        ...
        //pasteInto is not working so do a manual transfer
        for(int i = 0; i < maskPix.getWidth() * maskPix.getHeight(); i++){
            
            //get the X & Y from the index
            int x = i % maskPix.getWidth();
            int y = (i - x)/maskPix.getWidth();

            //check bounds in case the new mask is smaller
            if(x < newMask.getWidth() && y < newMask.getHeight()){
                newMask.setColor(x, y, ofColor(maskPix[i]));
            }
...
//        masterPix.setImageType(OF_IMAGE_GRAYSCALE);
        ...
        if(useMask){
            ...
            if(maskChanged){
                ...
                maskBoundEnd.set(0, 0);
                maskBoundStart.set(10000, 10000);
                
                int numWhite = 0;
                
                for(int i = 0; i < maskPix.getWidth() * maskPix.getHeight(); i++){
                    
                    //find all the bright pixels
                    if(maskPix[i] == 255){
                ...
                //if there are no white pixels in the mask then make the start and end 0,0
                if(numWhite == 0){
                    maskBoundEnd.set(0, 0);
                    maskBoundStart.set(0, 0);
                }
    ...
    //update the thread to have it update us with the new data
